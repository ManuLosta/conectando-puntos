...
async function readStockCSV(): Promise<any[]> {
  ...
  return dataLines.map(line => {
    const [codigo, descripcion, anul, cant, codprecan, resto, codpreres] = line.split(';');
    return {
      codigo: codigo?.trim(),
...
async function main() {
  ...
  const clientsData = [
    { name: 'Supermercado Don Pepe', email: 'compras@donpepe.com', address: 'Av. Belgrano 456', city: 'Buenos Aires' },
    { name: 'Almacén La Esquina', email: 'pedidos@laesquina.com', address: 'Rivadavia 789', city: 'Buenos Aires' },
    { name: 'MaxiKiosco Centro', email: 'ventas@maxikiosco.com', address: 'Córdoba 123', city: 'Buenos Aires' },
    { name: 'Mercadito del Barrio', email: 'info@mercadito.com', address: 'San Juan 456', city: 'Rosario' },
    { name: 'Almacén San José', email: 'contacto@sanjose.com', address: 'Mitre 789', city: 'Rosario' }
  ];
  ...
  for (let i = 0; i < 3; i++) {
    await prisma.clientDistributor.create({
      data: {
        ...
        paymentTerms: ['contado', '30 días', '15 días'][Math.floor(Math.random() * 3)]
      }
    });
  }
  ...
  for (const { product, originalData } of products.slice(0, Math.floor(products.length * 0.7))) {
